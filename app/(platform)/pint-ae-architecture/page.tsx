...
export default function PintAeArchitecture() {
  ...
  // Logic to determine UAE compliance rules/codes
  let computedInvoiceType = '380 (Standard Tax Invoice)';
  let computedProfileId = 'urn:peppol:bis:billing';
  let computedCustomizationId = 'urn:peppol:pint:billing-ae:1.0';
  ...
  if (isExport) {
    ...
    computedNoteRule = 'Specific deemed supply indicators must be declared at the invoice header. Tax Category S (5%) still applies unless exempt.';
  } else if (isFreeZone) {
    computedNoteRule = 'Out-of-Scope (Category O) or Zero-rated (Category Z) depending on whether trade is with mainland or freezone-to-freezone.';
  } else if (isProfitMargin) {
    computedNoteRule = 'Invoice must contain specific Arabic/English profit margin notices (e.g. "Profit Margin Scheme - Space for notice").';
  }

  const documentTypes = [
    { code: '380', type: 'Tax Invoice', desc: 'Standard B2B invoice where supplier and customer are registered tax entities in the UAE.' },
    { code: '381', type: 'Credit Note', desc: 'Issued to correct errors or reduce payable amounts in original invoices.' },
    { code: '388', type: 'Simplified Invoice', desc: 'Used for B2C transactions (retail). Buyer TRN is not required.' },
    { code: '480', type: 'Out of Scope Invoice', desc: 'Used for transactions outside the scope of UAE VAT (e.g. free zone transits).' }
  ];
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <div className="space-y-3">
        ...
        <h1 className="apple-h1">PINT AE Architecture</h1>
        <p className="text-sm text-textSecondary max-w-xl leading-relaxed">
          PEPPOL PINT AE is a localized customization of the PEPPOL International Invoice (PINT). It introduces specific document identification, transaction codes, and Arabized requirements.
        </p>
      </div>
      ...
      <div className="space-y-6">
        <h3 className="text-xs font-semibold text-textSecondary uppercase tracking-widest font-mono border-b border-white/[0.06] pb-2">
          UAE Transaction Flag Calculator
        </h3>
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center gap-3 cursor-pointer text-xs text-textSecondary hover:text-textPrimary">
                <input
                  ...
                  className="rounded bg-black border-white/20 text-accent focus:ring-accent"
                />
                <span>Free Trade Zone (FTZ) Sale</span>
              </label>
